...
func TestSourceStringsSurviveSanitization(t *testing.T) {
	worstCase := []string{
		strings.Join([]string{"wmi:Win32_QuickFixEngineering", "com:Microsoft.Update.Session"}, ", "), // windows
		strings.Join([]string{"file:/var/log/dpkg.log", "exec:apt"}, ", "),                            // linux (debian)
		strings.Join([]string{"exec:rpm", "exec:dnf"}, ", "),                                          // linux (rpm)
		strings.Join([]string{"exec:softwareupdate --history", "exec:softwareupdate -l"}, ", "),       // darwin
	}
