...
func (s *SocketServer) evaluateAndLaunch(req socketRequest, uid uint32, userID string) socketResponse {
	...
	if decision.Allowed {
		if result, launchErr := LaunchAsUser(uid, req.AppPath, splitArgs(req.CommandLine)); launchErr != nil {
			resp.Allowed = false
...
// This is a Phase 2 simplification with no shell-quoting support (an
// argument containing a literal space cannot be expressed) — acceptable for
// the common case of flag-style arguments; a fuller implementation would use
// a proper shell-word-splitting algorithm if quoted arguments are needed.
