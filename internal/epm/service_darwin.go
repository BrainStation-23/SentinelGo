...
// macOS — and manages its lifecycle. Callers (internal/main_integration.go)
// construct one Service per agent run when EPM is enabled; the same exported
// API exists on every platform (see service_windows.go, service_other.go) so
// the caller needs no build tags of its own.
type Service struct {
	socket *SocketServer
